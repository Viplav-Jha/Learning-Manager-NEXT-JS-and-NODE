...
export const getAllCourses = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const isCacheExits = await redis.get("allCourse");
      if (isCacheExits) {
        const courses = JSON.parse(isCacheExits);
        console.log("Hitting allcourse to redis");
        res.status(200).json({
          success: true,
          courses,
        });
        ...
        console.log("Hitting allcourse to mongodb");
        // added data to redis
...
export const getCourseByUser = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(new ErrorHandler("You are not eigible to course", 400));
      }
...
export const addReview = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (course) {
        course.rating = avg / course.reviews.length; //one example we have 2 reviews one is 5 another is 4 so math working like this = 9 /2 = 4.5 rating
      }
